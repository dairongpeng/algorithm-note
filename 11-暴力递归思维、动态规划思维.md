...
> 思路1：1. 先想办法把1~N-1层圆盘移动到中间杆，2. 再把N层的圆盘移动到最右侧的杆上 3. 把1~N-1个圆盘从中间杆移动到最右侧。结束

> 思路2：忘掉左中右，理解为从from移动到to，from和to都有可能是左中右。所以定义from，to，other三个杆子。1. 把1~N-1移动到other上。2. 把第N层移动到to上。3. 把1~N层从other移动到to上。结束
